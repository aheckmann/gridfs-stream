...
function GridWriteStream (grid, filename, options) {
  ...
  this._q = [];
  this._store = new mongo.GridStore(grid.db, this.id, this.name, this.mode, this.options);
}
...
GridWriteStream.prototype._flush = function _flush (_force) {
  ...
  var self = this;
  this._store.write(this._q.shift(), function (err) {
    if (err) return self._error(err);
    self.emit('progress', self._store.position);
    self._flush(true);
  });
}
